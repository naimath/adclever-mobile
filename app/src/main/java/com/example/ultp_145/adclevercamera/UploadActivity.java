...
public class UploadActivity extends Activity {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		try {
			...
			if (requestCode == RESULT_LOAD_IMG && resultCode == RESULT_OK
				...
				Uri uri = Uri.parse("content://media/external/images/media/21");

				// create a package provider string suggested by the error messge.
				String provider = "com.android.providers.media.MediaProvider";
			//	String provider = "com.example.ultp_145.adclevercamera";
	...
	public void makeHTTPCall() {
		...
		//http://192.168.2.5:9999/ImageUploadWebApp/uploadimg.jsp
		client.post("http://192.168.1.102:3000/avenues/upload1",
				params, new AsyncHttpResponseHandler() {
					...
					public void onFailure(int statusCode, Header[] headers, byte[] responseBody, Throwable error) {
						Log.i("printStack ",error.getMessage());
						error.printStackTrace();
						System.out.println("error message from "+error.getMessage());
						prgDialog.hide();
					}
